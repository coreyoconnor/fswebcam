...
int fswc_grab(fswebcam_config_t *config)
{
	uint32_t frame;
	uint32_t x, y;
    avgbmp_t *abitmaps[config->frames];
    avgbmp_t *pbitmaps[config->frames];
    ...
    memcpy(pbitmaps, abitmaps, sizeof(pbitmaps));
	...
	for(y = 0; y < config->height; y++) {
		for(x = 0; x < config->width; x++) {
			...
			int py = y;
			int out;
            double avg_h_x = 0.0, avg_h_y = 0.0, avg_s = 0.0, avg_v = 0.0;
            double valid_count = 0.0;

            for(frame = 0; frame < config->frames; frame++) {
                rgb rgb;

                rgb.r = (float)*pbitmaps[frame]++ / 255.0f;
                rgb.g = (float)*pbitmaps[frame]++ / 255.0f;
                rgb.b = (float)*pbitmaps[frame]++ / 255.0f;
                ...
                if (hsv.s <= 0.0f) {
                    ...
                    avg_h_x += cos(hsv.h / 180.0 * M_PI);
                    avg_h_y += sin(hsv.h / 180.0 * M_PI);
                    avg_s += hsv.s;
                    avg_v += hsv.v;
                    valid_count += 1.0;
                }
            ...
            double avg_h = 0.0;
            if (valid_count > 0.0) {
                avg_h_x = avg_h_x / valid_count;
                avg_h_y = avg_h_y / valid_count;
                avg_h = atan2(avg_h_y, avg_h_x);
                if (avg_h < 0.0) {
                    avg_h = (2.0 * M_PI + avg_h) / M_PI * 180.0;
                } else {
                    avg_h = avg_h / M_PI * 180.0;
                }
                avg_s = avg_s / valid_count;
                avg_v = avg_v / (float)config->frames;
            } else {
                avg_h = 0.0;
                avg_s = 0.0;
                avg_v = avg_v / (float)config->frames;
            }
            hsv avg_hsv = { (float)avg_h, (float)avg_s, (float)avg_v };

            rgb avg_rgb = hsv2rgb(avg_hsv);

            out = ((int)(avg_rgb.r * 255.0f) & 0xFF) << 16;
            out |= ((int)(avg_rgb.g * 255.0f) & 0xFF) << 8;
            out |= (int)(avg_rgb.b * 255.0f) & 0xFF;

			gdImageSetPixel(original, px, py, out);
		}
    }
